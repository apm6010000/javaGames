...
public class CalculateExpression {
	...
	/**
	 * 该对象拥有3个私有变量,因经常需使用,故在此定义 expression,用于记录表达式 isCorrect,表示表达式是否规范
	 * result,表达式计算结果
	 * 
	 * 其次,该对象拥有以下方法供外部调用 setExpression(String<表达式>) 输入表达式,无返回 getExpression()
	 * 返回表达式,类型为String isCorrect() 返回表达式是否规范,类型为boolean isCorrect(int<数字个数>)
	 * 返回表达式是否规范,此时式中的数字个数也为判断依据,类型为boolean getNumbers() 返回表达式中数字,类型为int[]
	 * getResult() 返回表达式的计算结果,类型为int getDoubleResult() 返回表达式的计算结果,类型为double
	 * compareNumbers(int[]<需对比的数组>) 比较表达式中的数字是否与给出的数相同,返回类型为boolean
	 * 
	 * 最后,该对象拥有以下私有方法 check() 判断表达式是否规范,并自动修正,返回类型为boolean (左右括号必须对齐,不可省略)
	 * calStep1(),calStep2(),calStep3() 计算表达式结果的三步,返回类型均为double
	 */

	/**
	 * 用于测试的main方法,正式编译时无用 ** public static void main(String[] args) {
	 * CalculateExpression n=new CalculateExpression(); String
	 * s=JOptionPane.showInputDialog
	 * (null,"请输入表达式","Test2",JOptionPane.QUESTION_MESSAGE); if (s==null) {
	 * System.exit(0); } n.setExpression(s);
	 * //JOptionPane.showMessageDialog(null
	 * ,n.getExpression(),"Test2",JOptionPane.INFORMATION_MESSAGE); if
	 * (!n.isCorrect()) //if (!n.isCorrect(4))
	 * JOptionPane.showMessageDialog(null
	 * ,"表达式有误","Test2",JOptionPane.INFORMATION_MESSAGE); //int[]
	 * num=n.getNumbers();
	 * //JOptionPane.showMessageDialog(null,""+num[0]+","+num
	 * [1]+","+num[2]+","+num[3],"Test2",JOptionPane.INFORMATION_MESSAGE); //int
	 * result=n.getResult(); double result=n.getDoubleResult();
	 * JOptionPane.showMessageDialog
	 * (null,"计算式的答案为"+result,"Test2",JOptionPane.INFORMATION_MESSAGE); }
	 */
	...
	private double calStep1() {
		...
		while (s.substring(0).indexOf('(') != -1) {
			...
			s.replace(startIndex, endIndex + 1,
					"" + calStep2(s.substring(startIndex + 1, endIndex)));
			if (!isCorrect) {
				return 0;
			}
		}
